...
public class GameDialogueDatabase : MonoBehaviour
{
    ...
    public List<AiFallbackData> fallbacks = new List<AiFallbackData>();
    ...
    private void LoadAllData()
    {
        ...
        fallbacks = LoadArrayJson<AiFallbackDataList, AiFallbackData>("Data/ai_fallback");
        ...
        Debug.Log($"[GameDialogueDatabase] Fallback 로드 수: {fallbacks.Count}");
    ...
    private List<TItem> LoadArrayJson<TWrapper, TItem>(string resourcePath) where TWrapper : class
    {
        ...
        if (wrapper is AiFallbackDataList fallbackList)
            return fallbackList.items as List<TItem>;
    ...
    public DialogueData GetDialogue(string speakerId, string sceneId, string dialogueType)
    {
        return dialogues.Find(x => x.speakerId == speakerId && x.sceneId == sceneId && x.dialogueType == dialogueType);
    }

    public DialogueData GetDialogueFallback(string speakerId, string dialogueType)
    {
        return dialogues.Find(x =>
            x.speakerId == speakerId &&
            x.dialogueType == dialogueType);
    }
    ...
    public List<DialogueData> GetDialogues(string speakerId, string sceneId, string dialogueType)
    {
        return dialogues.FindAll(x => x.speakerId == speakerId && x.sceneId == sceneId && x.dialogueType == dialogueType);
    }

    // 같은 타입의 대사 중 하나를 랜덤으로 반환
    public DialogueData GetRandomDialogue(string speakerId, string sceneId, string dialogueType)
    {
        List<DialogueData> candidates = GetDialogues(speakerId, sceneId, dialogueType);
    ...
    public AiFallbackData GetFallback(string triggerType, string npcId)
    {
        // 1순위: 현재 NPC 전용 fallback
        AiFallbackData npcFallback = fallbacks.Find(x =>
            x.triggerType == triggerType && x.speakerScope == npcId);

        if (npcFallback != null)
            return npcFallback;

        // 2순위: 공용 fallback
        return fallbacks.Find(x =>
            x.triggerType == triggerType && x.speakerScope == "all");
    }
}
